...
//		System.out.println(path+"music/badEnding.mp3");
		...
		image.imageTitle();
		time.timeDelay(1200);
		System.out.println("\n\n무슨 일이 생길 것처럼 미스터리한 냄새가 나는 밤...\n");
		time.timeDelay(1200);
		System.out.println("광주 동부 경찰서에 살인사건 제보가 들어왔다!\n");
		time.timeDelay(1200);
		System.out.println("당신은 살인사건의 범인을 밝혀낼 담당 형사!\n");
		time.timeDelay(1200);
		...
		while (true) {
			System.out.println("범인을 찾아낼 당신의 이름은??");
			System.out.println("[1] 회원가입 [2] 로그인");
			System.out.print("입력 >> ");
			choice = sc.next();

			if (choice.equals("1")) {
				System.out.println("회원가입! 새로운 형사님 환영합니다!!");
				while (true) {
					...
					boolean checkPbk = id.equals("pbk") || id.equals("PBK") || id.equals("Pbk")
							...
							|| id.equals("병관") || id.equals("갓병관") || id.equals("박병관쌤");

					if (checkPbk) {
						System.out.println("사용할 수 없는 아이디 입니다. 다른 아이디를 입력해주세요!\n");
				...
				while (true) {
					...
					boolean checkPbk = nick.equals("pbk") || nick.equals("PBK") || nick.equals("Pbk")
							...
							|| nick.equals("병관") || nick.equals("갓병관") || nick.equals("박병관쌤");

					if (checkPbk) {
						System.out.println("사용할 수 없는 닉네임 입니다. 다른 닉네임을 입력해주세요!\n");
					} else {
						break;
					}
				}
...
//				dao.getClue(2, id);
		...
		if (choice.equals("1")) {
			...
			System.out.println("그가 싸늘한 주검으로 발견되었다!");
			time.timeDelay(1200);
			System.out.println("당황한 상민은 피우려던 담배를 떨어뜨리고 시체 가까이 다가가는데...\n\n");
			time.timeDelay(1200);

			while (true) {
				System.out.println("시체를 확인하시겠습니까? [기본정보는 증거보기를 통해 확인할 수 있습니다]");
				...
				if (choice.equals("1")) {
					...
					image.imageIntro();
					System.out.println("\n피해자는 엎드린 채 죽어있다. 뒤통수를 살펴보니 큰 혹이 솟아있다. " + "누군가 뒤에서 그를 가격한 것일까?");
					time.timeDelay(1300);
					...
					image.imageWatch();
					System.out.println("\n피해자 시체 근처에 찌그러져 안이 보이는 선물 상자가 널부러져 있다. ");
					time.timeDelay(1200);
					System.out.println("상민이 열린 상자 안을 슬쩍 들여다 보니 반짝이는 시계가 보인다.");
					System.out.println("얼핏 보니 남자용 손목시계인 것 같다.\n");
					time.timeDelay(1200);
					// 초기 증거 선물상자 속 새 시계
				} else if (choice.equals("4")) {
					image.imagelip();
					System.out.println("\n혈흔과 헷갈리지만 묘하게 다른 빛의 자국이 피해자의 등판에 묻어있다. 입술 모양의 " + "자국... 이것은 무엇일까?\n");
					time.timeDelay(1300);
					// 초기 증거 립스틱 자국
				} else if (choice.equals("5")) {
					image.imageRing();
					System.out.println("\n피해자는 죽기전 어떤 여자와 마주친 것일까? 그의 바지춤 근처에 또 다른 물건이 보인다. "
							+ " 어둠속에서도 그 빛을 잃지 않은 꽤나 고가로 보이는 다이아반지를 발견한다. 상민은 흠칫 놀란다.\n");
					time.timeDelay(1300);
					// 초기 증거 다이아 반지
				} else if (choice.equals("6")) {
					System.out.println("\n상민은 놀란 가슴을 추스리고 옥상에서 벗어나 경찰에 신고한다.\n");
					System.out.println("[프롤로그 종료]\n 게임을 시작합니다.\n");
					break; // skip
				}
			}
		...
		while (true) {
			dao.save(cnt, id);
			System.out.println("\n당신은 복도에 도착했습니다.");
			System.out.println("[1] 탐색 [2] 범인지목 [3] 증거보기 [4] 용의자 보기 [5] 저장 [6] 종료 ");
			System.out.print("입력 >> ");
			choice = sc.next();
			cnt++;
			System.out.println("\n");
				...
				if (choice.equals("1")) {
					...
					while (true) {
						...
						if (choice.equals("1")) {
							...
							System.out.println("상민 : PBK선생님 발표 자료를 준비하다가 바람 좀 쐴 겸 올라간거고..\n");
							System.out.println("\t그전까지 저는 8시 40분 부터 시체 발견 전까지 여기서 계속 작업중이었습니다.");
							...
							if (dao.checkClue(4, id) == 4 || dao.checkClue(10, id) == 10) {
								...
								time.timeDelay(1300);
								System.out.println("상민 : 네? 지금 저를 의심하시나요? 매일 6시간씩 무급으로 근무하는 게 힘들었던 건 사실이지만...");
								System.out.println("\t저에게 좋은 취업자리를 주시기로 약속해서 무급이지만 열심히 일하고 있습니다!\n");
								System.out.println("\t이제와서 선생님을 죽이는 게 제게 무슨 득이 될까요? 선생님은 꼭 필요한 분입니다!! 저는 결백해요!\n");
								time.timeDelay(2000);
								System.out.println(nick + " : 알리바이를 증언해주실 분이 있으십니까? ");
								System.out.println("상민 : 그, 그건... ");
								time.timeDelay(1300);
								System.out.print(
										"아! 최근에 박나영원장님이랑 PBK선생님이 크게 다투는 것을 봤습니다! \n " + "\t박나영 원장님이 범인일지도 모릅니다!");
								time.timeDelay(1300);
...
//							 true(증거가 있을 때)
							// 추가정보
							music.playPushing();
							System.out.println(nick + " : 원장님, 질문 몇 가지만 드리겠습니다. (온갖 명품에.. 매우 화려한 스타일이군..)");
							System.out.println("나영 : 지금 원장인 저를 의심하시는건가요? 좋아요. 뭐든지 물어보세요.\n");
							time.timeDelay(1300);
							...
							if (dao.checkClue(5, id) == 5 || dao.checkClue(8, id) == 8 || dao.checkClue(11, id) == 11) {
								time.timeDelay(2000);
								System.out.println(
										"\n\n" + nick + " : 지금까지 알아본 바로는 최근에 나영씨와 pbk 사이에 좀 큰 문제가 있었는데, 그것과는.. ");
								System.out.println(
					...
					while (true) {
						...
						if (choice.equals("1")) {
							// 범인과의 대화 - 기본정보
							music.playPushing();
							System.out.println(nick + " : 장현도씨, 질문 몇 가지 드리겠습니다.(덩치가 크고... 생각보다 젊고 깔끔하군.)");
...
//						 true(증거가 있을 때)
							...
							System.out.println("현도 : 저는 PBK와 초등학생때부터 친구입니다. 저는 절대 범인이 아니에요.\n");
							time.timeDelay(1300);
					...
					while (true) {
						...
						if (choice.equals("1")) {
							// 범인과의 대화
							music.playPushing();
							System.out.println(nick + " : 서하라씨, 질문 몇 가지만 드리겠습니다.(불만이 많아보이는 군)");
							System.out.println("하라 : (짜증나는 표정) 혹시 PBK때문에 오셨나요?\n");
							time.timeDelay(1300);

							System.out.println(nick + " : 피해자와의 관계는 어떻습니까?");
							System.out.println("하라 : 저는 PBK와 대학동창이자 직장 동료예요. 솔직히 사이는 안 좋았어요.\n");
							time.timeDelay(1300);
							...
							if (dao.checkClue(7, id) == 7 || dao.checkClue(13, id) == 13) {
								...
								System.out.println("\n\n저한테 질문할 시간에 차라리 김지은 선생님을 조사해보는게 어때요?");
								time.timeDelay(1300);
								System.out.println(nick + " : 김지은 선생님? 그분은 왜 의심하시는 거죠?");
								System.out.println("하라 : 제가 전화하기 전에 계속 PBK를 찾아다녔어요. 평소에도 졸졸 쫓아다녀요."
										+ "\n\t제가 얼마전에 봤어요. 사실 PBK한테 스토커가 있었는데 분명히 그 여자에요. 평소에도 집착이 심하다구요.\n");
								time.timeDelay(1300);
								System.out.println(nick + " : 순찰하던 장현도씨와 마주쳤나요?");
								System.out.println("하라 : 네. 그 사람도 옥상 자주가요, 분명 또 담배 피러 갔겠죠.\n");
								time.timeDelay(1300);
								...
								System.out.println("하라 : 아니요 전 담배 냄새도 싫어서 담배 근처도 안 가는 사람이에요."
										+ "학생들이랑 같은 공간에 있어야 하는데 안 좋은 냄새를 풍기면 안되죠.");
					...
					while (true) {
						...
						if (choice.equals("1")) {
							// 범인과의 대화
							music.playPushing();
							System.out.println(nick + " : " + "김지은씨, 질문 몇 가지만 드리겠습니다.(화장이 진하네..)");
...
//							 범인과의 대화
							...
							time.timeDelay(1300);
							System.out.println(nick + " : " + "혹시 평소에도 빨간색 립스틱을 사용 하시나요?");
							...
							if (dao.checkClue(3, id) == 3 || dao.checkClue(9, id) == 9 || dao.checkClue(14, id) == 14) {
								...
								System.out.println("\n\n지은 : 형사님 그런데 교무실에 왔을때 옥상 열쇠가 없어져 있었어요.\n");//
								time.timeDelay(1300);
								System.out.println(nick + " : " + "옥상 열쇠는 지은씨가 관리하시는 건가요?");
								System.out.println(
										"지은 : 제가 관리하는 건 아니고 당직 선생님 매일 관리 한답니다. 항상 옥상 열쇠는 보관함에 있었는데 보니깐 없었어요.\n");
								time.timeDelay(1300);
								System.out.println("저는... 상민 학생이 의심가요... PBK 선생님 작업을 도와준다고는 하는데 너무 계속 붙어서 일을 하거든요..\n");//
								System.out.println(nick + " : 혹시 지은씨는 피해자 존경하는 마음 이상의 마음을 가지고 있나요?");
				...
				if (choice.equals("1")) {
					...
					if (criminal == 3) {
						...
						time.timeDelay(2000);
						System.out.println(
								"장현도는 PBK의 도움으로 경비원으로 취직했지만 거액의 빚으로 인해 \n" + "생활고에 시달리던 중, 결국 친구 PBK에게 거액의 돈을 빌린다.\n");
						time.timeDelay(2000);
						System.out.println(
								"처음에는 PBK의 도움으로 매우 고마워 했으나, \n" + "점점 자신을 하인 취급하고 무시하는 PBK의 태도에 큰 분노를 느끼게 된다.\n");
						time.timeDelay(2000);
						System.out.println("그러던 어느날 PBK가 빌려준 돈을 갚으라고 강하게 요구하고, \n "
								+ "장현도는 아직 갚을 돈 없다고 사정을 빌자 PBK는 거친욕설로 그를 맹비난한다.\n");
						time.timeDelay(2000);
						System.out.println("순간적인 분노로 장현도는 그를 강하게 밀었고, pbk는 벽에 뒤통수를 크게 부딪히며 기절하게 된다.\n"
								+ "그리고 당황한 그는 커터칼로 그의 경독맥을 절단하여 pbk를 살해하고 만다 .\n");
						time.timeDelay(2000);
						System.out.println("장현도는 휴게실 휴지통에 커터칼을 버려서 청소시간에 버려지도록 하고 " + "다른 사람이 용의자가 될 수 있게 옥상열쇠를 숨긴다.");
